...
* Eventually existing hidden track one audio ("HTOA") is merged with track 1 if it is 1 second long at maximum. If it is longer, it is kept as a separate track with number 0. This is done after AccurateRip checksum validation because AccurateRip checksums are always excluding HTOA.
